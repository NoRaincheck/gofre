...
func TestLoadValidConfig(t *testing.T) {
	...
	if cfg.Tool.GoFre.Module != "github.com/user/test-project" {
		t.Errorf("expected module 'github.com/user/test-project', got '%s'", cfg.Tool.GoFre.Module)
	}
	if cfg.Tool.GoFre.Bindings != "cffi" {
		t.Errorf("expected bindings 'cffi', got '%s'", cfg.Tool.GoFre.Bindings)
	}
	if cfg.Tool.GoFre.PkgDir != "pkg/core" {
		t.Errorf("expected pkg-dir 'pkg/core', got '%s'", cfg.Tool.GoFre.PkgDir)
	}
}
...
`
	...
	if cfg.Tool.GoFre.Bindings != "cffi" {
		t.Errorf("expected default bindings 'cffi', got '%s'", cfg.Tool.GoFre.Bindings)
	}
	if cfg.Tool.GoFre.PkgDir != "pkg" {
		t.Errorf("expected default pkg-dir 'pkg', got '%s'", cfg.Tool.GoFre.PkgDir)
	}
}
...
`
	...
	if len(cfg.Tool.GoFre.Binaries) != 3 {
		t.Fatalf("expected 3 binaries, got %d", len(cfg.Tool.GoFre.Binaries))
	}
	if cfg.Tool.GoFre.Binaries[0] != "cli" {
		t.Errorf("expected 'cli', got '%s'", cfg.Tool.GoFre.Binaries[0])
	}
}
...
func TestLoadBuildTags(t *testing.T) {
	...
	if len(cfg.Tool.GoFre.BuildTags) != 1 {
		t.Fatalf("expected 1 build tag, got %d", len(cfg.Tool.GoFre.BuildTags))
	}
	if cfg.Tool.GoFre.BuildTags[0] != "no_pocketpy" {
		t.Errorf("expected 'no_pocketpy', got '%s'", cfg.Tool.GoFre.BuildTags[0])
	}
}
...
`
	...
	if len(cfg.Tool.GoFre.BuildTags) != 0 {
		t.Errorf("expected 0 build tags by default, got %d", len(cfg.Tool.GoFre.BuildTags))
	}
}
